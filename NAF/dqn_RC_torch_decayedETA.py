...
# """.format(env.v_head, env.d0, RC, env.reward_mode, env.attack_mode, args.Nash))
    ...
    param_noise_attacker = AdaptiveParamNoiseSpec(initial_stddev=0.05,
                                                  ...
                                                  adaptation_coefficient=1.05) if args.param_noise else None
    try:
        res_data = pd.read_csv(suffix + '.csv')
    except:
        res_data = pd.DataFrame(columns=['Weight', 'Attack', 'Eva_distance'])

    try:
        reward_data = pd.read_csv(suffix + '_reward_.csv')
    except:
    ...
    for i_episode in range(args.num_episodes):
        if i_episode % 100 == 0 and i_episode != 0:
            ...
            reward_data.to_csv(suffix + '_reward_.csv', index=False)
        ...
        if args.NashMode == 0:
            ...
            ETA = 0.5 - i_episode/args.num_episodes * 0.5
        ...
        while True:
            ...
            if sigma > ETA:
                ...
                action_attacker = torch.Tensor(
                    [policy_attacker.predict(state_record[-1].reshape(-1, 4)) / policy_attacker.predict(
                        state_record[-1].reshape(-1, 4)).sum()])[0]
            # print(action_attacker.numpy()[0].shape)
            # print(action_attacker.numpy())
            # print(action_attacker.numpy().shape)
            ...
            if done:
                ...
                print('Episode {} ends, instant reward is {:.2f}, live step is {}'.format(i_episode, episode_reward, env.step_number))
        ...
        # distance_file.close()
    env.close()
    reward_data.to_csv(suffix+'reward_.csv', index=False)
    res_data.to_csv(suffix+'.csv', index=False)
